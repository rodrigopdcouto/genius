...
#define CHOICE_OFF 0   //Usado para controle dos LEDs
#define CHOICE_NONE 0  //Usado para verificar estado dos botões
...
#define BUZZER1 4
#define BUZZER2 7
// Conversão de definição do pino do Buzzer para inteiro
int soundMarch = BUZZER2;
...
void setup() {
  ...
  // Verifique se o botão inferior direito está pressionado (Amarelo)
  if (checkButton() == CHOICE_YELLOW) play_march();
  // Verifique se o botão superior direito está pressionado (Verde)
  ...
  play_winner();  // Após a conclusão da configuração, diga olá ao mundo piscando os LEDs
}
...
// Esta função é ativada quando o usuário segura o botão superior direito (Verde) durante a energização
...
void add_to_moves(void) {
  ...
  // Temos que converter esse número, 0 até 3, para as escolhas
...
// Reproduzir o som e as luzes do perdedor
...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// As seguintes funções estão relacionadas apenas ao Easter Egg de Star Wars
// Notas na melodia. Cada nota é repoduzida em oitavas.
void beep(unsigned char soundMarch, int frequencyInHertz, long timeInMilliseconds) {
  int x;
  long delayAmount = (long)(1000000 / frequencyInHertz);
  long loopTime = (long)((timeInMilliseconds * 1000) / (delayAmount * 2));
  for (x = 0; x < loopTime; x++) {
    digitalWrite(soundMarch, HIGH);
    delayMicroseconds(delayAmount);
    digitalWrite(soundMarch, LOW);
    delayMicroseconds(delayAmount);
  }
  delay(20);
  //Um pequeno atraso para fazer todas as notas soarem separadas
}

int LEDnumber = 0;  // Mantém o controle de qual LED estamos ligados durante o ciclo de star wars

void march(char oitava) {

  //Primeiro parte da marcha imperial
  beep(soundMarch, oitava * a, 500);
  changeLED();
  beep(soundMarch, oitava * a, 500);
  changeLED();
  beep(soundMarch, oitava * a, 500);
  changeLED();
  beep(soundMarch, oitava * f, 350);
  changeLED();
  beep(soundMarch, oitava * cH, 150);
  changeLED();
  beep(soundMarch, oitava * a, 500);
  changeLED();
  beep(soundMarch, oitava * f, 350);
  ...
  beep(soundMarch, oitava * cH, 150);
  changeLED();
  beep(soundMarch, oitava * a, 1000);
  changeLED();

  //Segunda parte da marcha imperial
  beep(soundMarch, oitava * eH, 500);
  changeLED();
  beep(soundMarch, oitava * eH, 500);
  changeLED();
  beep(soundMarch, oitava * eH, 500);
  changeLED();
  beep(soundMarch, oitava * fH, 350);
  changeLED();
  beep(soundMarch, oitava * cH, 150);
  changeLED();
  beep(soundMarch, oitava * gS, 500);
  changeLED();
  beep(soundMarch, oitava * f, 350);
  changeLED();
  beep(soundMarch, oitava * cH, 150);
  ...
  //Início da parte interessante
  beep(soundMarch, oitava * aH, 500);
  changeLED();
  beep(soundMarch, oitava * a, 350);
  changeLED();
  beep(soundMarch, oitava * a, 150);
  changeLED();
  beep(soundMarch, oitava * aH, 500);
  changeLED();
  beep(soundMarch, oitava * gSH, 250);
  changeLED();
  beep(soundMarch, oitava * gH, 250);
  changeLED();
  beep(soundMarch, oitava * fSH, 125);
  changeLED();
  beep(soundMarch, oitava * fH, 125);
  changeLED();
  beep(soundMarch, oitava * fSH, 250);
  changeLED();
  delay(250);
  beep(soundMarch, oitava * aS, 250);
  changeLED();
  beep(soundMarch, oitava * dSH, 500);
  changeLED();
  beep(soundMarch, oitava * dH, 250);
  changeLED();
  beep(soundMarch, oitava * cSH, 250);
  changeLED();

  //Ficando um pouco mais interessante
  beep(soundMarch, oitava * cH, 125);
  changeLED();
  beep(soundMarch, oitava * b, 125);
  changeLED();
  beep(soundMarch, oitava * cH, 250);
  changeLED();
  delay(250);
  beep(soundMarch, oitava * f, 125);
  changeLED();
  beep(soundMarch, oitava * gS, 500);
  changeLED();
  beep(soundMarch, oitava * f, 375);
  changeLED();
  beep(soundMarch, oitava * a, 125);
  changeLED();
  beep(soundMarch, oitava * cH, 500);
  changeLED();
  beep(soundMarch, oitava * a, 375);
  changeLED();
  beep(soundMarch, oitava * cH, 125);
  changeLED();
  beep(soundMarch, oitava * eH, 1000);
  changeLED();
  ...
  beep(soundMarch, oitava * f, 375);
  changeLED();
  beep(soundMarch, oitava * c, 125);
  changeLED();
  beep(soundMarch, oitava * a, 1000);
  changeLED();
}

// Esta função é ativada quando o usuário segura o botão inferior direito (Amarelo) durante a energização
void play_march() {
